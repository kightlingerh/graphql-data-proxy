...
import { CacheError, CacheResult, CacheWriteResult, Persist, Ref } from '../shared'
...
export interface Cache<R> {
	...
	toRef(variables: N.TypeOfMergedVariables<R>): CacheResult<Ref<N.ExtractModelType<R>>>
}
		...
		return <R extends N.SchemaNode<any, any>>(r: R) => {
			...
			if (isNonEmpty(errors)) {
				...
				return right<CacheError, Cache<R>>({
					...
					toRef: (variables) => toRefC(variables) as CacheResult<Ref<N.ExtractModelType<R>>>
				})
			}
		}
	}
