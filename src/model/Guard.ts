...
export const some = <A>(value: G.Guard<unknown, A>): G.Guard<unknown, Some<A>> =>
	G.type({
		_tag: G.literal('Some'),
		value
	})

export const option = <A>(value: G.Guard<unknown, A>): G.Guard<unknown, Option<A>> =>
	G.sum('_tag')({
		None: none,
		Some: some(value)
	})

export const left = <A>(left: G.Guard<unknown, A>): G.Guard<unknown, Left<A>> =>
	G.type({
		_tag: G.literal('Left'),
		left
	})

export const right = <A>(right: G.Guard<unknown, A>): G.Guard<unknown, Right<A>> =>
	G.type({
		_tag: G.literal('Right'),
		right
	})

export const either = <L, R>(l: G.Guard<unknown, L>, r: G.Guard<unknown, R>): G.Guard<unknown, Either<L, R>> =>
	G.sum('_tag')({
		Left: left(l),
		Right: right(r)
	})
