...
function useCache() {
	...
	const age = pipe(
		...
		fold(() => constant(shallowRef(none)), entries => () => entries.age)
	)()

	const weight = pipe(
		fromEither(cache),
		chain((c) => rightIO(c.toEntries({}))),
		fold(() => constant(shallowRef(none)), entries => () => entries.weight)
	)()

	const name = pipe(
		fromEither(cache),
		chain((c) => rightIO(c.toEntries({}))),
		fold(() => constant(shallowRef(none)), entries => () => entries.name)
	)()
