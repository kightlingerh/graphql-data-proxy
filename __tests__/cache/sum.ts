...
const HighSchoolEducation = N.type('HighSchoolEducation', {
	city: N.staticString,
	graduated: N.staticBoolean
}, { includeTypename: true });

const CollegeEducation = N.type('CollegeEducation', {
	city: N.staticString,
	graduated: N.staticInt,
	major: N.staticString
}, { includeTypename: true })

const Schema = N.schema('Query', {
...
function useCache() {
	...
	const write = (data: N.TypeOfPartial<typeof Schema>) => pipe(
		TE.fromEither(cache),
	...
	const city = computed(() => pipe(
		fromEither(cache),
		chain((c) => rightIO(c.toEntries({}))),
		fold(() => constant(shallowRef(none)) , e => () => e.education),
		IO.map(e => pipe(e.value, map(ed => ed.city), chainO(edc => edc.value)))
	)())
