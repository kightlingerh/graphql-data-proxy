...
const DemographicInfoNode = N.type('DemographicInfo', {
	age: N.staticInt,
	weight: N.staticInt,
	name: N.staticString
}, { includeTypename: true })

const EmploymentInfoNode = N.type('EmploymentInfo', {
...
describe('sum', () => {
	it('properly constructs strict model', async () => {
		await fc.assert(
			fc.asyncProperty(fc.oneof(fc.constant('EmploymentInfo' as const), fc.constant('DemographicInfo' as const)), fc.integer(), fc.integer(), fc.string(), fc.string(), async (__typename, age, weight, name, employer) => {
				const val = __typename === 'EmploymentInfo'
					? {__typename, employer }
					: { __typename, age, weight, name }
